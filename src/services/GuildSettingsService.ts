/**
 * GuildSettingsService - Direct implementation without over-abstraction
 */
...
export class GuildSettingsService {
  ...
  async setGuildSettings(guildId: string, settings: Partial<GuildSettings>): Promise<boolean> {
    try {
      const existingSettings = await this.getGuildSettings(guildId) || this.getDefaultSettings()
  ...
  async updateGuildSettings(guildId: string, updates: Partial<GuildSettings>): Promise<boolean> {
    try {
  ...
  async getAutoplayCounter(guildId: string): Promise<AutoplayCounter | null> {
    try {
      const counterData = await redisClient.get(this.getRedisKey(guildId, 'autoplay_counter'))
      if (!counterData) {
        return null
      }
  ...
  async setAutoplayCounter(guildId: string, counter: AutoplayCounter): Promise<boolean> {
    try {
      await redisClient.setex(
  ...
  async getRepeatCount(guildId: string): Promise<number> {
    try {
      const countData = await redisClient.get(this.getRedisKey(guildId, 'repeat_count'))
  ...
  async isRateLimited(guildId: string, command: string, cooldown: number): Promise<boolean> {
    try {
  ...
  async setRateLimit(guildId: string, command: string, cooldown: number): Promise<void> {
    try {
      const key = this.getRedisKey(guildId, `rate_limit:${command}`)
      await redisClient.setex(key, cooldown, Date.now().toString())
    } catch (error) {
  ...
  async clearAllAutoplayCounters(): Promise<boolean> {
    try {
      // Clear all autoplay counter keys from Redis
