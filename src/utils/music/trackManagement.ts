...
export async function addTrackToQueue(
    ...
    try {
        ...
        // Check if track is a duplicate
        if (await isDuplicateTrack(track, guildId, currentTrackIds)) {
...
export async function addTracksToQueue(
    ...
    try {
        ...
        const filteredTracks = await filterDuplicateTracks(
            tracks,
            guildId,
            currentTrackIds,
        )
...
export async function replenishQueue(
    ...
    try {
        ...
        const guildId = queue.guild.id
        const currentAutoplayCount = await getAutoplayCount(guildId)
        ...
        if (queue.tracks.size < 2 || forceReplenish) {
            ...
            if (tracksToAdd.length > 0) {
                ...
                const botUser = (queue.metadata as QueueMetadata)?.client?.user
                ...
                await incrementAutoplayCount(guildId, tracksToAdd.length)
