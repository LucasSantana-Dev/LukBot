...
export function extractArtistAndTitle(trackTitle: string): ArtistTitle {
    ...
    try {
        const match = artistTitlePatterns.find(pattern => pattern.test(trackTitle))?.exec(trackTitle);
 ...
 */
export function isSimilarTitle(firstTitle: string, secondTitle: string): boolean {
    if (!isValidTitle(firstTitle) || !isValidTitle(secondTitle)) return false;
    if (firstTitle === secondTitle) return true;

    try {
        const { artist: firstArtist, title: firstTitleOnly } = extractArtistAndTitle(firstTitle);
        const { artist: secondArtist, title: secondTitleOnly } = extractArtistAndTitle(secondTitle);
        ...
        if (normalizedFirstArtist && normalizedSecondArtist && normalizedFirstArtist === normalizedSecondArtist) return true;
        if (normalizedFirstTitle === normalizedSecondTitle) return true;

        // Check for substring matches with length threshold
        const minTitleLength = Math.min(normalizedFirstTitle.length, normalizedSecondTitle.length);
        if (minTitleLength > 10) { // Only check substrings for titles longer than 10 chars
            if (normalizedFirstTitle.includes(normalizedSecondTitle) || normalizedSecondTitle.includes(normalizedFirstTitle)) {
                return true;
            }
        }

        // Calculate similarity scores with weighted comparison
        const titleSimilarity = calculateSimilarity(normalizedFirstTitle, normalizedSecondTitle);
        const artistSimilarity = normalizedFirstArtist && normalizedSecondArtist ? 
            calculateSimilarity(normalizedFirstArtist, normalizedSecondArtist) : 0;

        // Weighted threshold based on whether we have artist information
        const titleThreshold = normalizedFirstArtist && normalizedSecondArtist ? 0.6 : 0.8;
