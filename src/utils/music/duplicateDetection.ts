...
export function addTrackToHistory(track: Track, guildId: string): void {
    try {
        ...
        debugLog({
            message: `Added track "${track.title}" to history for guild ${guildId}`,
        })
    } catch (error) {
...
export function isDuplicateTrack(
    ...
    try {
        ...
        if (recentlyPlayedTracks.get(guildId)?.some((t) => t.url === track.url))
            return true
        ...
        const isTitleSimilar = recentlyPlayedTracks.get(guildId)?.some((t) => {
            ...
            if (cleanedNewTitle.length < 5 || cleanedHistoryTitle.length < 5)
                return false
        ...
        if (lastTrack?.author && track.author) {
            ...
            if (normalizedLastArtist === normalizedCurrentArtist) {
                ...
                const recentArtistTracks = recentTracks.filter(
                    (t) =>
