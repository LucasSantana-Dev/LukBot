...
function createProgressEmbed(title, progress, stage, current, total) {
...
const command = new Command({
    data: new SlashCommandBuilder()
        ...
        .setRequired(true)
        .addChoices({ name: 'Video', value: 'video' }, { name: 'Audio', value: 'audio' })),
    execute: async ({ interaction }) => {
        ...
        try {
            ...
            const initialProgressEmbed = createProgressEmbed(video.title || 'Video', 0, 'downloading');
            ...
            heartbeatInterval = setInterval(async () => {
                try {
                    ...
                    await interaction.editReply({
                        ...
                        embeds: [embed, createProgressEmbed(video.title || 'Video', lastProgress, 'downloading')]
                    });
                }
                catch (error) {
            ...
            const downloadResult = await downloadVideo(videoUrl, format, async (progress) => {
                ...
                if (now - lastUpdate >= updateInterval || Math.abs(progress.progress - lastProgress) >= 1) {
                    ...
                    const progressEmbed = createProgressEmbed(video.title || 'Video', progress.progress, progress.stage, progress.current, progress.total);
            ...
            if (!downloadResult.success || !downloadResult.filePath) {
                ...
                const errorEmbed = createErrorEmbed(downloadResult.error || 'Failed to download the content.', interaction.user.id);
                await interaction.editReply({
                    content: null,
                    embeds: [errorEmbed]
                });
