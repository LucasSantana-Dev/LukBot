...
export async function downloadVideo(url, format, onProgress) {
    ...
    try {
        ...
        await promisify(require('fs').mkdir)(downloadFolder, { recursive: true });
        ...
        if (format === 'audio') {
            ...
            const estimatedSizePerMinute = (format === 'audio') ? 1024 * 1024 / 3 : 1024 * 1024;
            ...
            return new Promise((resolve, reject) => {
                ...
                audioBuffer.stream.on('error', (error) => {
                    ...
                    if (!streamStarted) {
                        ...
                        downloadWithAlternativeMethod(url, format, video, onProgress)
                            .then(resolve)
                            .catch(reject);
                    }
                });
                fileStream.on('finish', async () => {
                    ...
                    try {
                        ...
                        if (fileStats.size > MAX_FILE_SIZE) {
                            ...
                            await promisify(require('fs').rename)(tempFilePath, finalFilePath);
                        }
...
async function downloadWithAlternativeMethod(url, format, video, onProgress) {
    let downloadedBytes = 0;
    let totalBytes = 0;
    let progressInterval;
    try {
        ...
        const timestamp = Date.now();
        const extension = format === 'video' ? 'mp4' : 'mp3';
        const tempFilePath = join(downloadFolder, `temp_${title}_${timestamp}.${extension}`);
...
async function compressFile(inputPath, outputPath, format, onProgress) {
    return new Promise((resolve, reject) => {
...
export async function deleteDownloadedFile(filePath) {
    try {
